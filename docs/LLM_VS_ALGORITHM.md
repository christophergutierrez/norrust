...
| --- | --- | --- |
| **Greedy** | Each unit, in ID order: every reachable hex, expected-damage fight (`hit% × damage × strikes`, ×3 if that would kill), else walk toward the nearest enemy. Recruiters stay on a keep and may attack from it, or walk back if off keep. It has no team-level focus-fire plan, though sequential attacks can converge on one target. It does not value terrain, villages, or opponent replies. Fast. | `self-play --ai2 greedy`, or the headless LLM client described below. **Not** the Love2D “AI” controller. |

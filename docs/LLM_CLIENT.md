...
| `contact` | `stage: current_state`, `proposed_destination`, `proposed_placement`, or `proposed_placement_recruiter`; relevant unit/destination facts | Current or projected attack exposure requires model judgment. When `current_state`, includes bounded tactical options. When `proposed_destination`, routine code evaluated every stop on the unit's shortest route and each would leave a friendly or recruiter attackable on the next opponent turn; the evidence names the nearest such stop and adds `proposed_action` (the exact legal Move, not executed), `objective` (`kind` `village`, `castle_capacity` or `rally`, with `target` or null), `projected_threats` and `coverage`. `projected_threats.units` lists exposed friendlies with `is_mover`, `attacker_ids_any_view` (the union of attackers across both views) and separate `occupied` and `open` views; `recruiters` lists exposed recruiters once, with per-attacker `attacker_max_damage` in each view. `max_incoming_sum` is a sum of single-attack maxima and is not proven jointly achievable. At most 8 units are listed and `coverage.units_omitted` reports the rest. This stage never carries `contact_state_key` or `contact_actionability`, so it does not trigger contact closure, and `set_policy` stays available. Briefs render one causal line from these facts; missing values stay unknown. The same evidence carries a bounded menu in `options` (at most three) and the packet allows `choose` alongside `set_policy`, `act`, `finish_turn` and `resign`. `u<id>-proceed-1` (`proceed_with_exposure`) is exactly the rejected move; choosing it commits that legal Move with model authorship through the ordinary transaction and does not make it safe or clear later contact. Up to two `u<id>-safe-<n>` (`safe_alternative`) options pass the same whole-board projected safety check; they come from at most 16 nearby legal empty stops, excluding the current hex, the rejected stop and every shortest-route stop (all already proven unsafe), and `advances_objective` says whether each one gets closer to the objective (unknown when there is no target). `safe_search.status` is `complete`, `truncated` or `incomplete_error`; finding none means none within that bounded search, not that no safe move exists. There is no hold option: `finish_turn` leaves the unit in place and ends the whole controlled turn, and a `set_policy` listing the unit in `holds` lasts until the policy is replaced. This stage consumes no contact key and never triggers final-only closure. |
...
| `recruitment_review` | `queue_complete`, `gold`, `reserve_gold`, `unreserved_gold`, `placement_count`, `current_contact` | Completed finite recruitment queue with unreserved gold. Reconsider replenishment via `set_policy` or intentional saving without looping. |
...
| `recruitment_review` | `policy` | `set_policy`, `act`, `finish_turn`, `resign` | Completed queue with unreserved gold warrants replenishment or intentional saving. |
